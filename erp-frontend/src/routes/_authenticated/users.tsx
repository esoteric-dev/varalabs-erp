...
function UsersPage() {
  ...
  // Auto-suggest email when name changes
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Organisation members and their roles</p>
        </div>
        <button
          onClick={() => {
            setShowForm(true)
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-white rounded-lg border border-emerald-100 px-4 py-3">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-mono text-gray-800">{createdCredentials.email}</span>
                <button
            ...
            <div className="bg-white rounded-lg border border-emerald-100 px-4 py-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => copyToClipboard(createdCredentials.password, 'cred-pw')}
                  className="text-gray-400 hover:text-teal-600 transition-colors"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  type="email"
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent"
                  placeholder={orgSlug ? `name@${orgSlug}.com` : 'email@example.com'}
                />
              </div>
              <div>
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">System Role</th>
                <th className="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Org Roles</th>
              ...
              {users.map((user) => (
                <>
                  <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full capitalize ${roleColors[user.systemRole] || 'bg-gray-50 text-gray-600'}`}>
                  ...
                  {expandedUser === user.id && isTeacher(user) && (
                    <tr key={`${user.id}-classes`}>
                        ...
                        <div className="ml-7">
                          ...
                          <div className="flex flex-wrap gap-2 mb-3">
                            {classAssignments.map(ca => (
                              <span
                                key={ca.id}
                                className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded-lg bg-white border border-gray-200"
                              >
                                {ca.className}
                                {ca.isClassTeacher && (
                                  <span className="text-[9px] text-teal-600 font-semibold">CT</span>
                                )}
                                <button
                                  onClick={() => removeClassMutation.mutate(ca.id)}
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              className="px-2.5 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-teal-500 w-40"
                            />
                            <label className="flex items-center gap-1.5 text-xs text-gray-500">
                              <input
                                type="checkbox"
                                checked={newIsClassTeacher}
                                onChange={e => setNewIsClassTeacher(e.target.checked)}
