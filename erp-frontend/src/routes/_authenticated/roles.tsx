...
function RolesPage() {
  ...
  function startEditing(role: RoleWithPermissions) {
    ...
    setFormPermissions(role.permissions.map(p => p.code))
    setShowForm(true)
  }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent disabled:bg-gray-50 disabled:text-gray-400"
                  placeholder="e.g. librarian"
                />
              </div>
              <div>
                  ...
                  <div key={module} className="border border-gray-100 rounded-lg p-3">
                    ...
                    {perms.map((p) => (
                      <label key={p.id} className="flex items-center gap-2 text-xs text-gray-600 mb-1 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={formPermissions.includes(p.code)}
                          onChange={(e) => {
      ...
      {!orgId && !isLoading && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
          <p className="text-sm text-amber-700">
            Organisation context is required to view roles. This may not be available for your account type.
          </p>
        </div>
      )}
